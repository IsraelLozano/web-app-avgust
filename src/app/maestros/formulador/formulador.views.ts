import { IFormuladorDto } from './../../models/Maestras/IMaestraDto';
...
@Component({
  ...
  templateUrl: './formulador.views.html',
  styles: [],
})
export class FormuladorViews implements OnInit {
  ...
  GetData(filter: string) {
    ...
    this._maestraService
      .getListFormulador(filter)
      .pipe(finalize(() => loading.close()))
      .subscribe((resp) => {
        this.data = resp.map((p) => {
          return { id: p.IdFormulador, descripcion: p.NomFormulador, estado: p.estado };
        }) as ITipoGenerico[];
      });
  }
      ...
      if (resp?.event == 'Agregar') {
        this._dialogService
          ...
          .subscribe((result: boolean | undefined) => {
            ...
            const model: IFormuladorDto = {
              IdFormulador: dataReturn.id,
              NomFormulador: dataReturn.descripcion,
            ...
            if (result) {
              ...
              const loading = this.dialog.open(LoadingViews, { disableClose: true });
              this._maestraService
                .CreateOrUpdateFormulador(model)
                .pipe(finalize(() => loading.close()))
                .subscribe((resultado) => {
                  if (resultado) {
