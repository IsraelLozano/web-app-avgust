import { ITipoProductoDto } from './../../models/Maestras/IMaestraDto';
...
@Component({
  selector: 'app-id-tipo-producto',
  templateUrl: './id-tipo-producto.views.html',
  styles: [],
})
export class IdTipoProductoViews implements OnInit {
  data!: ITipoGenerico[];
  ...
  GetData(filter: string) {
    ...
    this._maestraService
      .getListIdTipoProducto(filter)
      .pipe(finalize(() => loading.close()))
      .subscribe((resp) => {
        this.data = resp.map((p) => {
          return { id: p.IdTipoProducto1, descripcion: p.NomTipoProducto, estado: p.estado };
        }) as ITipoGenerico[];
      });
  }
      ...
      if (resp?.event == 'Agregar') {
        this._dialogService
          ...
          .subscribe((result: boolean | undefined) => {
            ...
            const model: ITipoProductoDto = {
              IdTipoProducto1: dataReturn.id,
              NomTipoProducto: dataReturn.descripcion,
            ...
            if (result) {
              ...
              const loading = this.dialog.open(LoadingViews, { disableClose: true });
              this._maestraService
                .CreateOrUpdateIdTipoProducto(model)
                .pipe(finalize(() => loading.close()))
                .subscribe((resultado) => {
                  if (resultado) {
