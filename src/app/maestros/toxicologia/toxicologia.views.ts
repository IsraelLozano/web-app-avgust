import { IToxicologiaDto } from './../../models/Maestras/IMaestraDto';
...
@Component({
  selector: 'app-toxicologia',
  templateUrl: './toxicologia.views.html',
  styles: [],
})
export class ToxicologiaViews implements OnInit {
  data!: ITipoGenerico[];
  ...
  GetData(filter: string) {
    ...
    this._maestraService
      .getListToxicologica(filter)
      .pipe(finalize(() => loading.close()))
      .subscribe((resp) => {
        this.data = resp.map((p) => {
          return { id: p.IdToxicologica, descripcion: p.Descripcion, estado: p.estado };
        }) as ITipoGenerico[];
      });
  }
      ...
      if (resp?.event == 'Agregar') {
        this._dialogService
          ...
          .subscribe((result: boolean | undefined) => {
            ...
            const model: IToxicologiaDto = {
              IdToxicologica: dataReturn.id,
            ...
            if (result) {
              ...
              const loading = this.dialog.open(LoadingViews, { disableClose: true });
              this._maestraService
                .CreateOrUpdateToxicologica(model)
                .pipe(finalize(() => loading.close()))
                .subscribe((resultado) => {
                  if (resultado) {
