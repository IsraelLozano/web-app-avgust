import { ITipoDocumentoDto } from './../../models/Maestras/IMaestraDto';
...
@Component({
  ...
  templateUrl: './tipo-documento.views.html',
  styles: [],
})
export class TipoDocumentoViews implements OnInit {
  data!: ITipoGenerico[];
  constructor(
    ...
    this._maestraService
      ...
      .subscribe((resp) => {
        this.data = resp.map((p) => {
          return { id: p.IdTipoDocumento, descripcion: p.Nombre, estado: p.estado };
        }) as ITipoGenerico[];
      });
  }
      ...
      if (resp?.event == 'Agregar') {
        this._dialogService
          ...
          .subscribe((result: boolean | undefined) => {
            ...
            const model: ITipoDocumentoDto = {
              IdTipoDocumento: dataReturn.id,
              Nombre: dataReturn.descripcion,
            ...
            if (result) {
              ...
              const loading = this.dialog.open(LoadingViews, { disableClose: true });
              this._maestraService
                .CreateOrUpdateTipoDocumento(model)
                .pipe(finalize(() => loading.close()))
                .subscribe((resultado) => {
                  if (resultado) {
