import { ICientificaPlagaDto } from './../../models/Maestras/IMaestraDto';
...
@Component({
  selector: 'app-cientifico-plaga',
  templateUrl: './cientifico-plaga.views.html',
  styles: [],
})
export class CientificoPlagaViews implements OnInit {
  data!: ITipoGenerico[];
  ...
  GetData(filter: string) {
    ...
    this._maestraService
      .getListCientificoPlaga(filter)
      .pipe(finalize(() => loading.close()))
      .subscribe((resp) => {
        this.data = resp.map((p) => {
          return {
            id: p.IdNomCientificoPlaga,
            descripcion: p.NombreCientificoPlaga,
            estado: p.estado,
          };
        }) as ITipoGenerico[];
      });
      ...
      if (resp?.event == 'Agregar') {
        this._dialogService
          ...
          .subscribe((result: boolean | undefined) => {
            ...
            const model: ICientificaPlagaDto = {
              IdNomCientificoPlaga: dataReturn.id,
              NombreCientificoPlaga: dataReturn.descripcion,
            ...
            if (result) {
              ...
              const loading = this.dialog.open(LoadingViews, { disableClose: true });
              this._maestraService
                .CreateOrUpdateCientificoPlaga(model)
                .pipe(finalize(() => loading.close()))
                .subscribe((resultado) => {
                  if (resultado) {
